...
class ActorFactory {
    ...
    public push(actorType: ActorType, actorObject: any)
    {
        ...
        if(this.actorConstructors[actorType]) throw `Actor implementation for type '${actorType}' already exists`;

        // First we check it really is a function
        if(typeof actorObject !== "function") throw "Actor implementation is not valid";
    ...
    getInstance(actorType: ActorType)
    {
